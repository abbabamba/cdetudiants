...
export default async function ProfilEtudiantPage() {
  ...
  let pe: {
    statut_verification: string; photo_url?: string | null
    universite?: string | null
  } | null = null

  if (isPreview) {
    profile = MOCK_ETUDIANT
    pe = MOCK_PROFIL_ETUDIANT
  } else {
    let { data: p } = await supabase
      .from('profiles').select('*, telephone, phone_verified').eq('id', user.id).single()

    if (!p) {
      await new Promise(r => setTimeout(r, 1000))
      const retry = await supabase.from('profiles').select('*, telephone, phone_verified').eq('id', user.id).single()
      p = retry.data
    }
    ...
    if (p) {
      const { data: peData } = await supabase
        .from('profils_etudiants').select('*').eq('user_id', user.id).single()
      pe = peData
    }
  }

  const displayEmail = isPreview ? MOCK_ETUDIANT.email : user.email!
  ...
  return (
    <div className="page-enter" style={{ maxWidth: '600px', margin: '0 auto', padding: 'clamp(24px,5vw,48px) 24px' }}>
      ...
      <div className="anim-fade-up" style={{
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: '18px', marginBottom: '24px' }}>
          ...
          <div style={{
            ...
            background: pe?.photo_url ? 'transparent' : 'var(--blue)',
            ...
            {pe?.photo_url
              ? <img src={pe.photo_url} alt="Photo" style={{ width: '100%', height: '100%', objectFit: 'cover' }} />
          ...
          <div style={{ flex: 1 }}>
            <h1 style={{
              ...
              fontSize: '22px', fontWeight: 700, color: 'var(--navy)', margin: '0 0 6px',
            }}>
          ...
          {!isPreview && (
            <Link href="/etudiant/profil/completer" style={{
              display: 'flex', alignItems: 'center', justifyContent: 'center',
        ...
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '16px' }}>
          ...
          {pe?.universite && <InfoItem label="Université" value={pe.universite} />}
          ...
          {profile?.phone_verified && profile.telephone ? (
            <div>
              ...
              <div style={{ display: 'flex', alignItems: 'center', gap: '6px', flexWrap: 'wrap' }}>
                ...
                <span style={{ background: 'var(--green-light)', color: 'var(--green)', fontSize: '10px', fontWeight: 700, padding: '2px 7px', borderRadius: '99px' }}>
                  ✓ Vérifié
                </span>
              </div>
            </div>
            ...
            <div>
              ...
              <div style={{ display: 'flex', alignItems: 'center', gap: '6px' }}>
                <span style={{ fontSize: '14px', fontWeight: 500, color: 'var(--text)' }}>{profile.telephone}</span>
                {!isPreview && (
                  <Link href="/etudiant/profil/telephone" style={{ background: '#FEF3C7', color: '#92400E', fontSize: '10px', fontWeight: 700, padding: '2px 7px', borderRadius: '99px', textDecoration: 'none' }}>
                    ⚠ Vérifier
                  </Link>
                )}
              </div>
      ...
      {/* ── 2. STATUT VÉRIFICATION ────────── */}
      {pe && (
        <div className="anim-fade-up stagger-2 card" style={{ marginBottom: '16px' }}>
          ...
          <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
            <BadgeVerifie statut={statut} size="md" />
            <span style={{ fontSize: '12px', color: 'var(--text-muted)', textAlign: 'right', maxWidth: '180px' }}>
              {statut === 'verifie_email'  && 'Email universitaire confirmé'}
              {statut === 'verifie_admin'  && 'Validé par notre équipe'}
              {statut === 'en_attente_admin' && 'Examen en cours · 48h'}
              {statut === 'rejete'          && 'Document refusé'}
              {statut === 'non_verifie'     && 'Non encore vérifié'}
            </span>
          </div>
          {!isPreview && (statut === 'non_verifie' || statut === 'rejete') && (
            <Link href="/etudiant/profil/completer" className="btn-primary shine-on-hover" style={{ width: 'auto', padding: '8px 18px', marginTop: '14px', fontSize: '13px' }}>
      ...
      {!isPreview && !pe && (
        <div style={{
          ...
          <p style={{ margin: '0 0 8px' }}>Profil en cours de création…</p>
          <RefreshButton />
        </div>
      )}
      ...
      {!isPreview && (() => {
        ...
        if (!profile?.telephone || !profile.phone_verified) {
          items.push({ icon: '📱', label: 'Vérifier mon téléphone', desc: 'Accédez aux coordonnées des bailleurs', href: '/etudiant/profil/telephone', badge: 'Recommandé' })
        }
        if (!pe?.photo_url) {
          items.push({ icon: '📷', label: 'Ajouter une photo', desc: 'Humanisez votre profil', href: '/etudiant/profil/completer' })
        }
        if (!pe?.universite) {
          items.push({ icon: '🎓', label: 'Ajouter mon université', desc: 'Aide les bailleurs à vous identifier', href: '/etudiant/profil/completer' })
        }
        if (!profile?.ville) {
          items.push({ icon: '📍', label: 'Ajouter ma ville', desc: 'Pour des annonces pertinentes', href: '/etudiant/profil/completer' })
        }
        ...
        return (
          <div className="anim-fade-up stagger-3 card">
      ...
      {/* Version preview : profil complet simulé */}
      {isPreview && (
        <div className="anim-fade-up stagger-3" style={{
          background: 'var(--green-light)', border: '1px solid rgba(45,122,58,0.2)',
          borderRadius: '14px', padding: '16px 20px',
